...
agent_communication:
  ...
  - agent: "main"
    message: "Two fixes based on user feedback: (a) Sherpa agent must live ON the canvas, not as a separate sidebar/panel item. (b) Blurs looked weird, need to be adjusted intelligently (colors are fine, keep blue/purple). Changes: (1) Removed the Ask Sherpa item from the sidebar entirely — the sidebar is now a clean icon rail of 7 items (Dashboard/Studio/Templates/Automations/Skills/Codebase/Docs) with no agent slot. (2) New `CanvasSherpaBar` component created — a floating pill input that lives at the bottom-center INSIDE the Studio canvas (absolute positioned, z-20, above the ReactFlow). Sparkle icon on the left, input field, send arrow button on the right. On focus expands to show suggestion chips. On submit calls the existing runSherpaSend() pipeline and opens the right panel to the Sherpa conversation. Shows an in-flight status strip (with stop button) while the agent is generating. This mirrors Cursor/Sim.ai's inline AI bar pattern. Removed the 'Ask Sherpa' button from the Studio empty state (the canvas bar is always there instead). (3) Intelligent blur adjustments: corner-glow-tr radius reduced 700x480 → 520x360, opacity 28% → 22% (tighter, more refined). Login page glow blobs reduced from blur-[120px]/blur-[100px] to blur-[70px]/blur-[60px] with smaller blob sizes. AIBuild section corner glow blur-2xl → blur-xl and opacity 60% → 45%. Marketing nav scroll backdrop-blur-xl → backdrop-blur-md with slightly more solid bg (85% instead of 70%). Studio generating veil no longer uses backdrop-blur — now uses a solid card with border. All blurs feel more polished, less hazy."